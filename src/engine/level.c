...
u8 collision_result;
char text[10]; // Corrigido para corresponder ao header
...
static u32 items_table[NUMBER_OF_ROOMS][NUMBER_OF_32BIT_BITMAPS] = {0}; // Corrigido para usar NUMBER_OF_32BIT_BITMAPS

static void LEVEL_remove_tile_from_buffer(u16 x, u16 y, u8 new_index);
void LEVEL_generate_screen_collision_map(u8 empty, u8 first_wall, u8 last_wall); // Removido static
...
// Alterado para retornar void para corresponder ao header
...
*/
    ...
    for (u16 y = 0; y < SCREEN_TILES_H; y += 2) {
        ...
        if (tile_index == empty) { // empty -> 0
            ...
            (tile_index >= first_wall && tile_index <= last_wall) ||
            (tile_index == IDX_TOCA_DO_RATO && !pegou_queijo) // Toca é sólida só se não pegou o queijo
        ) {
...
static void LEVEL_scroll_and_update_collision(s16 offset_x, s16 offset_y) {
    // >> COLLISION MAP (16x16)
    // << ROOM BIT MAP (16x16)
    LEVEL_register_tiles_in_room(LEVEL_current_room());
    ...
    #ifdef DEBUG_LEVEL
    kprintf("Obtaing map region 40x28 from SGDK Map for room %d", LEVEL_current_room());
    #endif
    // >> SGDP COMPRESSED MAP
    // << MAP RECT (8x8)
    ...
    // << MAP RECT (8x8)
    LEVEL_restore_tiles_in_room(LEVEL_current_room());
...
////////////////////////////////////////////////////////////////////////////
// DRAWING AND FX
// Removidas as implementações das funções inline que já estão no header
